...
from src.gsop.values.terminology.artifact import MAX_NUM_STATS, ArtifactEnum
...
class WeightedAttrs:
    ...
    def get(self, weighted_attr: AttributeEnum):
        return self._weighted_attrs.setdefault(weighted_attr, 0)
...
def _artifact_rating_expectation_4stats(lvl: int, attrs: ArtifactAttrs,
    ...
    rating = 0
    chance_per_stat = chances / MAX_NUM_STATS
    for attr in attrs.attrs():
        tmp = attrs.get_scale(attr)
        tmp += chance_per_stat * 0.85
        rating += tmp * weights.get(attr)
    return rating


def artifact_rating_expectation(artifact_type: ArtifactEnum, mainstat: AttributeEnum, lvl: int,
    ...
    chances = artifact_remaining_enhance(lvl)
    rating = 0
    chance_per_stat = (chances - MAX_NUM_STATS + attrs.num_of_attrs()) / MAX_NUM_STATS
    for attr in attrs.attrs():
        tmp = attrs.get_scale(attr)
        tmp += chance_per_stat * 0.85
        rating += tmp * weights.get(attr)
    if attrs.num_of_attrs() != MAX_NUM_STATS:
        backup_stats = artifact_type.substat_weights()
        backup_stats.pop(mainstat, None)
        for attr in attrs.attrs():
            backup_stats.pop(attr, None)
        for attr in weights.attrs():
            if attr in backup_stats:
                rating += wd_p_key(backup_stats, attr) * weights.get(attr) * (
                        1 + chance_per_stat) * 0.85
    return rating


def best_possible_rating(artifact_type: ArtifactEnum, mainstat: AttributeEnum, lvl: int,
                         attrs: ArtifactAttrs, weights: WeightedAttrs) -> float:
    chances = artifact_remaining_enhance(lvl)
    rating = 0
    pass
...
def default_rating_to_crit_based_rating(rating: float) -> float:
    return rating * AttributeEnum.CRIT_DMG.substat_max_val() * 100
